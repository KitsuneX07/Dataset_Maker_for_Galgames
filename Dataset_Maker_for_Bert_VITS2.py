    ...
    with open(list_file_path, 'w', encoding='utf-8') as list_file:
        ...
        for file_name in os.listdir(search_dir):
            ...
            if os.path.splitext(file_name)[0] in file_names:
                ...
                # 在列表文件中添加一行
                list_file.write(f'./Data/{output_dir_name}/wavs/{output_dir_name}/{os.path.splitext(file_name)[0]}.wav|{output_dir_name}|JP|{next(item["Text"] for item in data if item["File"] == os.path.splitext(file_name)[0])}\n')

# 使用示例
copy_files(r'E:\gal\workspace\output\aid.json', r'E:\gal\workspace\voice', 'aid', 'short_character_anno.list')