                ...
                if ke_line.startswith('koePlay'):
                    ...
                    if match:
                        if len(match.group(1)) == 6:
                            ...
                            path = os.path.join(r"D:\ghast\downloads\KANON\KANON\KANON_SE\KOE", folder_number, 'Z000' + match.group(1) + '.ogg')
                        else:
                            folder_number = '00' + match.group(1)[0:2]
                            path = os.path.join(r"D:\ghast\downloads\KANON\KANON\KANON_SE\KOE", folder_number, 'Z00' + match.group(1) + '.ogg')
                        ...
                        if pos:
                            ...
                            for utf_line in utf_lines:
                                if utf_line.startswith(pos):
                                    if "\\m{A}" in utf_line:
                                        utf_line = utf_line.replace("\\m{A}", "ゆういち")
