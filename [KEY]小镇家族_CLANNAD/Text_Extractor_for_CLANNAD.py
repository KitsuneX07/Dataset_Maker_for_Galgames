        ...
        with open(ke_path, 'r', encoding='utf-8') as ke_f, open(utf_path, 'r', encoding='utf-8') as utf_f:
            ...
            for i in range(len(ke_lines)):
                ...
                if ke_line.startswith('koePlay'):
                    match = re.search(r'\((\d+)\)', ke_line)
                    if match:
                        file_name = 'Z' + match.group(1).zfill(9) + '.ogg'
                        folder_number = file_name[1:5]
                        path = os.path.join(r'D:\ghast\downloads\CLANNAD\KOE', folder_number, file_name)
                        ...
                        if pos:
                            ...
                            for utf_line in utf_lines:
                                if utf_line.startswith(pos):
                                    if "\\m{B}" or "\\m{A}" in utf_line:
                                        utf_line = utf_line.replace("\\m{B}", "ともや")
                                        utf_line = utf_line.replace("\\m{A}", "ともや")

                                    if "\\l{A}" in utf_line or "\\l{B}" in utf_line:
                                        break
